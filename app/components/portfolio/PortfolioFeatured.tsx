...
export default function PortfolioFeatured({
  ...
  const featured =
    caseStudy ||
    CASE_STUDIES.find((cs) => cs.id === "unbox") ||
    CASE_STUDIES[0];
    ...
    switch (cs.id) {
      default:
        return {
          ...
          execution: cs.pillars?.[0]?.points?.[0] || "Execution completed.",
          outcome: cs.systemicResult || cs.result,
          categoryBadge: cs.category.toUpperCase(),
          desc: cs.title,
          stats:
            cs.performanceMetrics?.slice(0, 3).map((m) => ({
              label: m.label,
              value: m.value,
            })) || [],
        };
  ...
  return (
    <section
      ...
      className="relative bg-transparent py-20 border-b border-white/5 overflow-hidden"
    >
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none opacity-40 select-none">
        ...
        >
          ...
          />
          <path
            d="M 100 150 Q 400 50 700 150 T 1200 150"
            fill="none"
